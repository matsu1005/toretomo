<template>
  <v-dialog v-model="planDialog" max-width="500px">
    <v-card rounded="lg">
      <div v-if="existsMessages" class="pt-10">
        <server-alert />
      </div>
      <v-card-title style="display: flex">
        <div
          v-if="plan.user_id !== $store.state.currentUser.user.id"
        ...
        <div v-if="plan.user_id !== currentUser.id" style="margin: 0 0 0 auto">
          <v-btn
            v-if="isJoined"
          ...
          >
            参加中
            <v-icon dark> mdi-run-fast </v-icon>
          </v-btn>
          <v-btn
            v-else
            :disabled="!$auth.loggedIn"
