<template>
  <v-form ref="form">
    <div>
      <v-row v-show="show || user.name == 'ゲストユーザー1'" justify="center">
        <p cols="12" class="mt-3 display-1">{{ user.name }}さんのマイページ</p>
      </v-row>
      <server-alert />
      <v-row v-show="!show && user.name !== 'ゲストユーザー1'" justify="center">
        ...
        <p cols="12" class="mt-3 display-1">さんのマイページ!</p>
      </v-row>
        ...
        <v-card outlined class="pa-5" style="display: flex; border: none">
          <div class="mr-10" style="text-align: center">
            ...
            <div v-show="show" style="text-align: center">
              ...
              <p
                v-if="user.birth_year && user.birth_year !== 'null'"
                class="mb-0"
              >
                <small
                  >{{ user.birth_year }}年{{ user.birth_month }}月{{
                    user.birth_day
                  }}日</small
                >
              </p>
              <p v-else class="mb-0"><small>未設定</small></p>
              <p class="mb-0">
                <small
                  >性別 :
                  {{
                    user.sex && user.sex !== "null" ? user.sex : "未設定"
                  }}</small
                >
              </p>
            </div>
            <div v-show="!show" style="width: 250px">
              <p class="mb-0"><small>生年月日 :</small></p>
          ...
          <div v-if="show" style="max-width: 600px">
          ...
          <v-card-text v-if="!show" style="min-width: 600px">
            <p><strong>プロフィール</strong></p>
            <v-textarea
  ...
  data() {
    return {
      ...
      userInfo: {
        ...
        profile:
          this.$store.getters["currentUser/user"].profile == "null"
            ? ""
