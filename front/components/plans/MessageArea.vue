<template>
  <div>
    <v-container style="min-height:200px">
      <p><strong>トークスペース</strong>
      <v-btn
        small
        dense 
        class="mb-2"
        @click="scrollToEnd()">下まで移動する</v-btn>
      </p>
      <v-list three-line style="padding:0px">
        <template >
          <v-list-item 
            ...
            :key="index" 
          >        
            <nuxt-link style="text-decoration: none;" :to="{ path: `/users/${msg.user_id}` }">
              <v-avatar
                color="#445CB0"
                size="45"
                class="mr-2"
            ...
            <div>
              ...
              <small style="color:gray">{{$dayjs(msg.created_at).format('YYYY-MM-DD HH:mm')}}</small>
            </div>
          </v-list-item>
        </template>
      </v-list>
      <v-btn
        small
        dense 
        @click="scrollToTop()">上まで移動する</v-btn>
    </v-container>
    <v-footer
      color="transparent"
      height="60"
